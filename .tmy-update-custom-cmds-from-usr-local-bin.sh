...
docker cp /usr/local/bin/tmy-persist-custom-commands-through-image temp-for-image-commit-${EPOCH_TIME}:/usr/local/bin/tmy-persist-custom-commands-through-image
...
docker cp /usr/local/bin/tmy-pstorm-launch-GITPOD_REPO_ROOT ${EPOCH_TIME}:/usr/local/bin/tmy-pstorm-launch-GITPOD_REPO_ROOT
